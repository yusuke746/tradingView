...
ENTRY_FILTER_PROMPT_PREFIX: str = (
    ...
    "Trigger sources (see trigger.source in ContextJSON):\n"
    "  PRIMARY  : LiquiditySweep — direct sweep event.\n"
    "  SECONDARY: ZonesTouch gated by a recent Sweep in cache.\n"
    "  RETRIGGER: Lorentzian entry_trigger when an active Sweep exists in cache (ENABLE_LORENTZIAN_RETRIGGER). Score it like any other entry; the Sweep-gate already validated directionality.\n"
    "You MUST NOT output BUY/SELL, only score quality.\n"
    "setup_grade in ContextJSON (assigned BEFORE AI — describes signal quality, not AI output):\n"
    "  A+  = LiquiditySweep (base TTL) + ZonesTouch (sync_ok) + (FVG or MTF aligned). Strongest confluent setup.\n"
    "  A   = Three equivalent paths: (1) Sweep+Zone [sync fail or no FVG/MTF], (2) Extended Sweep(×2.5 TTL)+Zone+sync_ok, (3) Zone+FVG+MTF (sweep-less fallback). Standard setup.\n"
    "  B   = Two paths: (1) Extended Sweep (×5.0 TTL) + Zone only, (2) Lorentzian re-entry when active Sweep exists (LR_RETRIG, zone-exempt). Reduced-conviction setup — LRR executes at 50% lot automatically; AI scoring still applies normally.\n"
    "  C   = Sweep-only (no Zone) low-risk slot. Lowest-conviction setup — LRR executes at 25% lot automatically; AI scoring still applies normally.\n"
    "  REJECT = blocked before reaching AI. Never seen by this prompt.\n"
    "Treat A+ as strongest, A as standard, B as frequency-extension opportunity (lower conviction, lot-discounted 50% by LRR), C as lowest-conviction sweep-only slot (lot-discounted 75% by LRR).\n"
    ...
    "3. Q-TREND (LORENTZIAN CLASSIFIER) ALIGNMENT — QUALITY SIGNAL: Once MATH and STRUCTURE are acceptable, this determines which score TIER the setup belongs to.\n"
    "   Note: 'Lorentzian' here refers to the Q-Trend direction/strength signal (ML classifier), NOT the trigger source.\n"
    ...
    "Entry trigger (Sweep/ZonesTouch/Lorentzian-retrigger) + Q-Trend STRONG ALIGNED → HIGH QUALITY SETUP:\n"
    ...
    "  → Grade B setup: same tier targets apply; lot is already discounted 50% by LRR, do NOT lower score for that reason.\n"
    "  → Grade C setup: same tier targets apply; lot is already discounted 75% by LRR, do NOT lower score for that reason.\n\n"
    "Entry trigger + Q-Trend NORMAL ALIGNED → STANDARD SETUP:\n"
    ...
    "Entry trigger + Q-Trend MISALIGNED → COUNTER-TREND PENALTY:\n"
    ...
    "- Prefer fewer but higher quality trades; still allow valid B-grade and C-grade opportunities above threshold.\n"
    ...
    "=== SIGNAL FRESHNESS SCORING ADJUSTMENT ===\n"
    "Apply the following score adjustments AFTER computing the base confluence score.\n"
    "These are additive penalties; they do NOT override Kill Switches.\n\n"
    "sweep_age_sec (confluence.sweep_age_sec in ContextJSON):\n"
    "  0-120s:    no penalty\n"
    "  121-300s:  -5 points\n"
    "  301+s:     -12 points\n"
    "  null/missing: no penalty (treat as fresh)\n\n"
    "sync_delta_sec (confluence.sync_delta_sec in ContextJSON):\n"
    "  0-90s:    no penalty\n"
    "  91-180s:  -5 points\n"
    "  181+s:    -10 points\n"
    "  null/missing: no penalty\n\n"
    "setup_path penalty multiplier (confluence.setup_path in ContextJSON):\n"
    "  If setup_path is 'B', 'LR_RETRIG', or 'C': multiply ALL freshness penalties above by 1.5.\n"
    "  Other paths (A_PLUS, A_P1, A_P2, A_P3): no multiplier.\n"
    "  Example: sweep_age_sec=350, setup_path='B' -> penalty = -12 x 1.5 = -18 points.\n\n"
...
ENTRY_LOGIC_MINIMAL_PREFIX: str = (
    ...
    "Trigger source (see trigger.source): LiquiditySweep (PRIMARY), ZonesTouch gated by recent Sweep (SECONDARY), or Lorentzian retrigger when Sweep in cache (RETRIGGER).\n"
    "Environment: Q-Trend (Lorentzian classifier) provides direction/strength context only (not a trigger).\n"
)
