...
ENTRY_FILTER_PROMPT_PREFIX: str = (
    ...
    "- SMA乖離 (Mean Reversion Check): distance_atr_ratio (乖離率) を厳密に評価せよ。\n"
    "  0.0〜3.0倍: 健全なトレンド (Healthy Trend)。順張りエントリーに最適。Mean Reversion Riskは低いとみなせ。\n"
    "  3.0〜5.0倍: 過熱気味 (Extended)。トレンドは強いが反発に警戒。lot_multiplier を 1.0 以下に抑える。\n"
    "  5.0倍以上 (CRITICAL): 行き過ぎ (Overextended)。平均回帰リスクが極めて高い。順張りは原則REJECT。\n"
...
CLOSE_LOGIC_PROMPT_PREFIX: str = (
    ...
    "    - System threshold is constraints.min_close_confidence (65), but Phase 1 育成 requires extra margin → aim for 80+ to CLOSE, <=60 to HOLD.\n"
    ...
    "      0.0〜3.0倍: 健全 (Healthy) → 反転に過敏になりすぎない。\n"
    "      3.0〜5.0倍: 過熱 (Extended) → 反転リスクに注意し、利確をやや優先。\n"
    "      5.0倍以上: 行き過ぎ (Overextended) → 平均回帰リスク高。利益確保を優先して早めにクローズ検討。\n"
